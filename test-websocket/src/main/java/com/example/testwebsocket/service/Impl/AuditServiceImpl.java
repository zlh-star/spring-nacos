...
import com.example.testwebsocket.handle.TestConfig;
...
//        indexRequests.forEach(testConfig.createBulkProcessor()::add);
    ...
    @Async("taskExecutor")
    @Override
    public CompletableFuture<User> inLog(User demoDto) {
        //异步单条插入
        CompletableFuture.completedFuture(demoDto).whenComplete((res,ex)-> {
            if(ex!=null){
                try {
            ...
            indexRequest.source(JSON.toJSONString(res), XContentType.JSON)
                    .opType(DocWriteRequest.OpType.CREATE)
